...
// Performs a prefix search against the keys in the trie.
func (t *Trie) PrefixSearchNodes(pre string) []*Node {
	node := findNode(t.Root(), []rune(pre))
	if node == nil {
		return nil
	}

	return collectNode(node)
}
...
func collectNode(node *Node) []*Node {
	var (
		n *Node
		i int
	)
	keys := make([]*Node, 0, node.termCount)
	nodes := make([]*Node, 1, len(node.children))
	nodes[0] = node
	for l := len(nodes); l != 0; l = len(nodes) {
		i = l - 1
